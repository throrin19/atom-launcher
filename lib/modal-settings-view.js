...
export default class ModalSettingsView {
    ...
    _initModal() {
        try {
            this.configs = JSON.parse(fs.readFileSync(path.join(atom.project.getPaths()[0], '.atom', 'atom-launcher.json')));
        } catch (e) {
            this.configs            = [];
        }
        this.currentConfig      = null;
        this.hasOneModified     = false;
    ...
    _createSelectList() {
        ...
        this.configs.forEach((config) => {
            this._createEntryBtn(list, config);
        });
    }
    ...
    _createNewEntryBtn(list) {
        ...
        btn.addEventListener('click', () => {
            ...
            if (this.hasOneModified) {
                return;
            }

            this.hasOneModified = true;
    ...
    _showForm({ btn, config, list }) {
        ...
        if (this.config.envs) {
            this.config.envs.forEach((data) => {
                this._createEnvLine({ indexEnvs, form, data });
            });
        }
    ...
    _createEnvLine({ indexEnvs, form, data }) {
        ...
        inputVal.classList.add('input-text', 'native-key-bindings');
        inputName.classList.add('input-text', 'native-key-bindings');
    ...
    _onChangeCurrentConfig() {
        if (!this.hasOneModified) {
            const itemList = this.listContainer.querySelector(`li[data-id="${this.config.id}"]`);

            if (itemList) {
                itemList.childNodes[0].textContent += '*';
            }
        }
